  ...
  return <div className="competencies-page">
    ...
    {loading && !data ? <div className="panel loading-panel">Считаем показатели по текущим данным…</div> : data && <>
      {activeSection === 'gaps' && <>
        ...
        <div className="table-wrap"><table><thead><tr><th>Навык</th><th>Тип</th><th>Требуется людям</th><th>С разрывом</th><th>Доля</th><th>Критических</th></tr></thead>{displayedSkills.map(skill => <tbody key={skill.skill_id}><tr className="clickable-row" onClick={() => setExpanded(expanded === skill.skill_id ? '' : skill.skill_id)}><td><b>{skill.name}</b></td><td>{skill.type === 'hard' ? 'Hard' : 'Soft'}</td><td>{skill.required_count}</td><td><b className={skill.gap_count ? 'gap' : 'ok'}>{skill.gap_count}</b></td><td>{skill.gap_pct == null ? '—' : `${numberLabel(skill.gap_pct, 1)}%`}</td><td>{skill.critical_count}</td></tr>{expanded === skill.skill_id && <tr className="expanded-row"><td colSpan={6}><div className="skill-detail"><div><h3>Затронутые сотрудники</h3>{skill.people.length ? <div className="table-wrap"><table><thead><tr><th>Сотрудник</th><th>Сейчас</th><th>Цель</th><th>Разрыв</th><th>Каталог</th></tr></thead><tbody>{skill.people.map(person => <tr key={person.employee_id}><td><Link to={`/hr/employees?id=${encodeURIComponent(person.employee_id)}`}>{person.full_name}</Link><small className="muted block">{person.employee_id}</small></td><td>{person.current}</td><td>{person.required}</td><td>{person.gap}</td><td>{coverageLabels[person.coverage_status] || person.coverage_status}</td></tr>)}</tbody></table></div> : <p className="empty">Нет сотрудников с разрывом по фильтру.</p>}</div><div><h3>Мероприятия каталога</h3>{skill.events.length ? <ul className="event-list">{skill.events.map(event => <li key={event.event_id}><b>{event.title}</b><small>{event.coverage_status === 'partial_ceiling' ? 'Имеет потолок прироста; влияние зависит от цели' : coverageLabels[event.coverage_status] || event.coverage_status}</small></li>)}</ul> : <p className="empty">В каталоге нет мероприятия, развивающего этот навык.</p>}</div></div></td></tr>}</tbody>)}</table>{!skills.length && <div className="empty">По выбранным фильтрам навыков нет.</div>}</div>
