  ...
  return <div className="hr-review">
    ...
    {tab === 'recommendations' && <div className="hr-review-recommendations">
      <section className="panel recommendation-section">
        ...
        <div className="card-list">{recommendations.length ? recommendations.map(item => <article className="recommendation-card" key={item.event_id}>
          ...
          <p>{item.rationale}</p>
          <details className="fact-details"><summary>Данные и основания выбора</summary><div className="tags"><span>{item.event_id}</span><span>{item.type}</span><span>{formatLabels[item.format] || item.format}</span><span>{numberLabel(item.duration_hours, 1)} ч</span><span>{item.format === 'self_paced' ? 'В своём темпе' : item.session_date ? `Сессия ${dateLabel(item.session_date)}` : 'Дата уточняется'}</span></div>{item.tradeoff && <p className="tradeoff"><b>Ограничение выбора:</b> {item.tradeoff}</p>}<ul>{item.factors.map(factor => <li key={factor.id}>{factor.label} <small>{factor.source}</small></li>)}</ul></details>
      ...
      <section className="panel hr-review-archive-list"><div className="section-heading"><div><span className="eyebrow">АРХИВ HR</span><h2>Убранные предложения</h2></div><span className="muted">{profile.archives.length}</span></div>
        {profile.archives.length ? <div className="card-list">{profile.archives.map(item => <article className="compact-card" key={item.event_id}><div><strong>{item.title || item.event_id}</strong><small>{item.event_id} · Причина HR: {item.reason}</small></div><button className="button subtle" disabled={!!busy} onClick={() => void act(`restore-${item.event_id}`, () => api.restoreArchive(employeeId, item.event_id), 'Предложение восстановлено из архива HR.')}>Восстановить</button></article>)}</div> : <p className="empty">Архивных предложений нет.</p>}
    ...
    {tab === 'participation' && <div className="hr-review-participation">
      <section className="panel"><div className="section-heading"><div><span className="eyebrow">ТЕКУЩЕЕ УЧАСТИЕ</span><h2>Запланировано и в работе</h2></div><span className="muted">{active.length}</span></div>
        {active.length ? <div className="card-list">{active.map(item => <article className="participation-card" key={item.id}><div><span className="eyebrow">{item.event_id} · {statusLabels[item.status] || item.status}</span><h3>{item.title}</h3><p className="muted">{item.session_date ? `Сессия ${dateLabel(item.session_date)}` : 'В своём темпе'}{item.is_simulated ? ' · смоделировано' : ''}</p></div></article>)}</div> : <p className="empty">Нет запланированных или начатых активностей.</p>}
      </section>
      ...
      <p className="footnote">Attended означает завершённое участие, Skipped — добровольный выбор сотрудника без штрафа. Неявки и отказы от обязательного назначения имеют отдельные статусы.</p>
    </div>}
  </div>
}
