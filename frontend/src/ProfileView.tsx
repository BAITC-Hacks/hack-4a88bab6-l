...
export function JournalSection({ rows, filter, onFilter, from, onFrom, to, onTo, expanded, onExpanded }: {
  ...
  return <section className="panel history-section">
    ...
    <div className="table-wrap"><table><thead><tr><th>Активность</th><th>Дата записи</th><th>Статус</th><th>Прогресс</th></tr></thead><tbody>{visible.slice(0, expanded ? undefined : 12).map(row => row.kind === 'activity' ? <tr key={row.key}><td><b>{row.entry.title}</b>{(row.entry.mandatory || row.entry.is_simulated) && <small className="muted block">{row.entry.mandatory ? 'Обязательное' : ''}{row.entry.mandatory && row.entry.is_simulated ? ' · ' : ''}{row.entry.is_simulated ? 'Смоделировано' : ''}</small>}</td><td>{dateLabel(row.entry.date)}{row.entry.date_meaning === 'enrollment_or_assignment' && <small className="muted block" title="Для самостоятельного курса исходная дата может означать зачисление, а не завершение">дата зачисления</small>}</td><td><span className={`status ${row.entry.status}`}>{row.entry.status === 'completed' ? 'Attended · Завершено' : statusLabels[row.entry.status] || row.entry.status}</span></td><td>{row.entry.completion_pct}%</td></tr> : <tr key={row.key}><td><b>{row.entry.title || 'Активность'}</b><small className="muted block">{row.entry.reason || 'Причина не указана'}</small></td><td>{dateLabel(row.date)}<small className="muted block">дата выбора</small></td><td><span className="status skipped">Skipped · Пропущено</span></td><td>—</td></tr>)}</tbody></table>{!visible.length && <div className="empty">{filter === 'skipped' ? 'Активных добровольных пропусков за выбранный период нет.' : filter === 'attended' ? 'Завершённых активностей за выбранный период нет.' : 'Событий за выбранный период нет.'}</div>}</div>
    ...
    <p className="hint">Skip — добровольный выбор без штрафа и без изменения навыков. Неявка и отказ от назначения остаются отдельными статусами участия. Для самостоятельных курсов дата исходной записи может означать зачисление, а не точный день завершения.</p>
  </section>
}
  ...
  return <div className="profile-view employee-dashboard">
    ...
    <section className="panel recommendation-section" id="recommendations">
      ...
      {tab === 'recommended' && <div className="card-list">{recommendations.length ? recommendations.map(item => <article className="recommendation-card" key={item.event_id}>
        ...
        <details className="fact-details"><summary>Подробнее о выборе</summary><ul>{item.factors.map(f => <li key={f.id}>{f.label}</li>)}</ul></details>
      ...
      {tab === 'chosen' && <div className="card-list">{active.length ? active.map(item => <article className="participation-card" key={item.id}><div><span className="eyebrow">{statusLabels[item.status] || item.status}</span><h3>{item.title}</h3><p className="muted">{item.session_date ? `Сессия ${dateLabel(item.session_date)}` : 'В своём темпе'}{item.is_simulated ? ' · учебная демонстрация' : ''}</p></div>{mode === 'employee' && <div className="rec-actions">{(profile.demo_mode || !item.session_date || item.session_date <= profile.as_of_date) && <button className="button primary" disabled={!!busy} onClick={() => void act(`complete-${item.id}`, () => api.complete(item.id, Boolean(profile.demo_mode && item.session_date && item.session_date > profile.as_of_date)), 'Выполнение сохранено; навыки пересчитаны.', 'completed')}>{profile.demo_mode && item.session_date && item.session_date > profile.as_of_date ? 'Смоделировать выполнение' : 'Отметить выполненной'}</button>}<button className="button subtle" disabled={!!busy} onClick={() => void act(`stop-${item.id}`, () => api.stop(item.id), 'Участие прекращено и сохранено в истории.')}>Прекратить участие</button></div>}</article>) : <div className="empty">Пока нет выбранных или начатых активностей.</div>}</div>}

      {tab === 'completed' && <div className="card-list">{completed.length ? completed.map(item => <article className="compact-card" key={item.id}><div><strong>{item.title}</strong><small>{dateLabel(item.date)}{item.mandatory ? ' · обязательное' : ''}{item.is_simulated ? ' · смоделировано' : ''}</small></div><span className="badge positive">Выполнено</span></article>) : <div className="empty">В выбранном периоде завершённых активностей нет.</div>}</div>}

      {tab === 'skipped' && <div className="card-list">{profile.skips.length ? profile.skips.map(item => <article className="compact-card" key={item.event_id}><div><strong>{item.title || 'Активность'}</strong><small>{item.reason || 'Причина не указана'} · Добровольный пропуск не меняет навыки</small></div>{mode === 'employee' && <button className="button subtle" disabled={!!busy} onClick={() => void act(`restore-${item.event_id}`, () => api.restoreSkip(item.event_id), 'Предложение возвращено в рассмотрение. Обновите подбор.', 'recommended')}>Вернуть в рассмотрение</button>}</article>) : <div className="empty">Пропущенных предложений нет.</div>}</div>}

      {tab === 'archived' && <div className="card-list">{profile.archives.length ? profile.archives.map(item => <article className="compact-card" key={item.event_id}><div><strong>{item.title || 'Активность'}</strong><small>Причина HR: {item.reason}</small></div>{mode === 'hr' && <button className="button subtle" disabled={!!busy} onClick={() => void act(`unarchive-${item.event_id}`, () => api.restoreArchive(employeeId!, item.event_id), 'Архив восстановлен. Пропуск сотрудника, если он был, остаётся.', 'recommended')}>Восстановить</button>}</article>) : <div className="empty">Архивных предложений нет.</div>}</div>}
    </section>
    ...
    <details className="panel details-panel"><summary>Все навыки выбранной цели <span className="muted">{profile.skills.filter(s => s.required > 0).length}</span></summary>
      <div className="table-wrap"><table><thead><tr><th>Навык</th><th>Тип</th><th>Сейчас</th><th>Требуется</th><th>Разрыв</th><th>Критический</th></tr></thead><tbody>{profile.skills.filter(s => s.required > 0).sort((a,b) => b.gap - a.gap).map(s => <tr key={s.skill_id}><td><b>{s.name}</b></td><td>{s.type === 'hard' ? 'Hard' : 'Soft'}</td><td>{s.current}</td><td>{s.required}</td><td><span className={s.gap ? 'gap' : 'ok'}>{s.gap}</span></td><td>{s.critical ? 'Да' : '—'}</td></tr>)}</tbody></table></div>
    </details>
